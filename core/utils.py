...
def generate_image(frame, args, generator):
    ...
    if args.aux_folder == '':
        folder = './samples/{}'.format(args.dataset_name)
    else:
        folder = './samples/{0}/{1}/{2}'.format(args.dataset_name,args.aux_folder,args.seed)
    if not os.path.exists(folder):
        os.makedirs(folder)
    lib.save_images.save_images(samples, folder + '/samples_{}.jpg'.format(frame))
...
def toy_true_dist_kde(args, true_dist):
    ...
    if args.aux_folder == '':
        ...
        folder = './samples/{}/'.format(args.dataset_name) + args.aux_folder
    if not os.path.exists(folder):
        os.makedirs(folder)
