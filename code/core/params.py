...
class Parameters:
    def __init__(self, parser):
        ...
        # Save Results
        if self.aux_folder == '':
            self.savefolder = '../results/' + 'seed:' + str(self.seed)
        else:
            self.savefolder = '../results/' + self.aux_folder + '_seed:' + str(self.seed)
        if not os.path.exists(self.savefolder):
            os.makedirs(self.savefolder)
        # self.aux_folder = '../record/results/Auxiliary/'
        # if not os.path.exists(self.aux_folder):
        #     os.makedirs(self.aux_folder)

        self.writer = SummaryWriter(log_dir=os.path.join(self.savefolder, 'tensorboard'))
